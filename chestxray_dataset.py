    ...
    def string(self):
        ...
        for i, d in enumerate(self.datasets):
            if i < len(self.datasets) - 1:
                s += "â”œ{} ".format(i) + d.string().replace("\n", "\n|  ") + "\n"
            else:
                s += "â””{} ".format(i) + d.string().replace("\n", "\n   ") + "\n"
        return s
...
class SubsetDataset(Dataset):
    ...
    def __getitem__(self, idx):
        return self.dataset[self.idxs[idx]]
class NIH_segmentation_3D_Dataset(Dataset):
    def __init__(self,
                 imgpath,
                 seed=0,
                 train=True,
                 ):
        super(NIH_segmentation_3D_Dataset, self).__init__()
        np.random.seed(seed)  
        self.imgpath = imgpath
        self.targets=[]
        self.pathlist=[]
        self.target_suffix='_gt.nii.gz'
        self.input_suffix = '_img.nii.gz'
        self.dis_suffix = '_dis.pt'
        for i in os.listdir(os.path.join(self.imgpath,'imgs')):
            self.pathlist.append(i.strip('_img.nii.gz'))
    ...
    def __getitem__(self, idx):
        ...
        f_idx=self.pathlist[idx]
        img_sitk = sitk.ReadImage(os.path.join(self.imgpath,'imgs', f_idx+self.input_suffix))
        image_data = sitk.GetArrayFromImage(img_sitk)
        
        gt_sitk = sitk.ReadImage(os.path.join(self.imgpath, 'gts', f_idx + self.target_suffix))
        gt_data = sitk.GetArrayFromImage(gt_sitk)
        dis_data = torch.load(os.path.join(self.imgpath, 'diss_once', f_idx + self.dis_suffix))[0]
        img = default_transform(image_data)
        img = intensity_transform(img)
        label = default_transform(gt_data)
        distribution=default_transform(dis_data)
        mask=[]
        for i in range(label.shape[1]):
            if torch.max(label[:,i])==torch.min(label[:,i]):
                mask.append(0)
            else:
                mask.append(1)
    ...
    def get_mask_dict(self, image_name, this_size):
        ...
        return path_mask
class NIH_segmentation_Dataset(Dataset):
    def __init__(self,
                 imgpath,
                 seed=0,
                 train=True,
                 ):
        super(NIH_segmentation_Dataset, self).__init__()
        np.random.seed(seed)  
        self.imgpath = imgpath
        self.targets=[]
        self.imagepathlist=[]
        for i in os.listdir(os.path.join(self.imgpath,'CXR_png')):
            if i.strip('.png')+'_mask.png' not in os.listdir(os.path.join(self.imgpath,'masks')) and i.strip('.png') + '.png' not in os.listdir(os.path.join(self.imgpath, 'masks')):
                continue
            self.imagepathlist.append(i.strip('.png'))
            if i.strip('.png') + '_mask.png' in os.listdir(os.path.join(self.imgpath, 'masks')):
                self.targets.append(i.strip('.png')+'_mask')
            elif i.strip('.png') + '.png'  in os.listdir(os.path.join(self.imgpath, 'masks')):
                self.targets.append(i.strip('.png'))
            else:
                raise ValueError
        ...
        if train:
            self.augmentations = albumentations.Compose(
                [
                    ...
                    albumentations.Normalize(mean=[0.5], std=[0.5],always_apply=True),
                ]
            )
        else:
            self.augmentations = albumentations.Compose(
    ...
    def __getitem__(self, idx):
        ...
        sample["lab"] = self.targets[idx]
        label_idx=self.targets[idx]
        mask_path = os.path.join(self.imgpath, 'masks',label_idx + '.png')
        mask_label=cv2.resize(imread(mask_path),(self.sz,self.sz))

        imgid = self.imagepathlist[idx]
        img_path = os.path.join(self.imgpath,'CXR_png',imgid+'.png')
        image = cv2.resize(imread(img_path),(self.sz,self.sz))
        ...
        if self.augmentations is not None:
            ...
            print('there',np.max(image), np.min(image))
        image = np.expand_dims(image, axis=0).astype(np.float32)
        mask_label = np.expand_dims(mask_label, axis=0).astype(np.float32)
        
        
        return (
            torch.tensor(image, dtype=torch.float32),
            torch.tensor(mask_label, dtype=torch.float32),
        )
    ...
    def get_mask_dict(self, image_name, this_size):
        ...
        for i in range(len(images_with_masks)):
            ...
            if row["Finding Label"] in self.pathologies:
                ...
                path_mask[self.pathologies.index(row["Finding Label"])] = mask
        return path_mask
class NCT_dataset(Dataset):
    def __init__(self, dataset_root,logits_VIT,logits_RN50, train=True,transform=None):
        """
        Args:
        ...
        self.path_list=[]
        self.target=[]
        self.extra_matrix_VIT = logits_VIT
        self.extra_matrix_RN50 = logits_RN50
        self.dataset_root=dataset_root
        if train==True:
            self.train='train'
        else:
            self.train='test'
        for i in sorted(os.listdir(os.path.join(self.dataset_root,self.train))):
            self.path_list.append(i)
            cls_name = i.strip('.tif').split('-')[0]
            cls = np.array(CLASS_NAMES[cls_name])
    ...
    def __getitem__(self, index):
        img_path, target,logit_VIT,logit_RN50 = self.path_list[index], self.target[index],self.extra_matrix_VIT[index],self.extra_matrix_RN50[index]
        img = tiff.imread(os.path.join(self.dataset_root,self.train,img_path))
        ...
        return (
            ...
            logit_VIT,logit_RN50
        )

class NIH_binary_Dataset(Dataset):
    ...
    def __init__(self,
        ...
        super(NIH_binary_Dataset, self).__init__()

        np.random.seed(seed)  
        self.imgpath = imgpath
        self.targets=[]
        self.imagepathlist=[]
        for i in os.listdir(os.path.join(self.imgpath,'NORMAL')):
            self.imagepathlist.append("NORMAL/"+i)
            self.targets.append(np.int64(0))
        for i in os.listdir(os.path.join(self.imgpath, 'PNEUMONIA')):
            self.imagepathlist.append("PNEUMONIA/" + i)
            self.targets.append(np.int64(1))
    ...
    def __getitem__(self, idx):
        ...
        return (
            torch.tensor(image, dtype=torch.float32),
            torch.tensor(sample["lab"], dtype=torch.int64),
        )
...
class NIH_Dataset(Dataset):
    ...
    def __init__(self,
        ...
        self.targets=self.csv['transfer_label']
        self.imageids=self.csv['Image Index']
        sz = 224
        if train:
            self.augmentations = albumentations.Compose(
                [
    ...
    def __getitem__(self, idx):
        sample = {}
        sample["idx"] = idx
        sample["lab"] = self.targets.iloc[idx]
        imgid = self.imageids.iloc[idx]
        img_path = os.path.join(self.imgpath, imgid)
        image = imread(img_path)
        if len(image.shape)==3:
            image=image[:,:,0]
        ...
        for i, pathology in enumerate(self.pathologies):
            for group_name, df_group in rows.groupby("class_name"):
                if (group_name == pathology) or (
                        (pathology in self.mapping) and (group_name in self.mapping[pathology])):
